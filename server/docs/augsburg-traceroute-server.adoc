...
with no global upper limit. But due to the current limited _eBPF_ functionality, this is not yet feasible.
 ...
 *--allow-indirect-from*=<path>::
    ...
    Only requests from client inside one of the specified networks are processed. +
    Clients are matched against this list only after they passed the general whitelist. +
    If not explicitly specified this defaults to */etc/augsburg-traceroute-server/[v4|v6]/allowed_indirect.txt*. +
    If no network entries are present in the file, requests from all addresses are allowed.

== Examples

To run the server on the interface 'eth0' with default parameters:
[example]
augsburg-traceroute-server-v4 eth0

To run the server with maximum 10.000 sessions and a session timeout of 5.5 seconds:
[example]
augsburg-traceroute-server-v4 -n 10000 -t 5.5 eth0

To use TCP probes without the SYN flag set:
[example]
augsburg-traceroute-server-v4 --tcp-syn-probes=no eth0

To allow clients requesting indirect traces to other targets:
[example]
augsburg-traceroute-server-v4 --indirect=yes eth0

To specify a custom whitelist of networks from which request can be made:
[example]
augsburg-traceroute-server-v4 --allow-from=./my_networks.txt eth0
