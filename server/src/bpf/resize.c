...
/*
 * Resizes the packet to be able to hold the specified length on top of the IPv4 header.
 */
INTERNAL int resize_l3hdr(struct cursor *cursor, __u16 probe_len, struct ethhdr **eth, struct iphdr **ip)
{
    int ret = bpf_skb_change_tail(cursor->skb, sizeof(**eth) + sizeof(**ip) + probe_len, 0);
