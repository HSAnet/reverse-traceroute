 ...
 * On a valid configuration state is created and a traceroute probe sent back to the originator.
 * Otherwise a response notifying the originator about the invalid configuration is dispatched.
 */
static int handle_request(struct cursor *cursor, struct ethhdr **eth, struct iphdr **ip, struct icmphdr **icmp)
{
    int err;
    union trhdr *tr;
