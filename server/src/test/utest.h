...
#endif
#endif
    return UTEST_CAST(utest_int64_t, ts.tv_sec) * 1000 * 1000 * 1000 +
           ts.tv_nsec;
#elif __APPLE__
...
template <> struct utest_type_deducer<short, false> {
    static void _(const short s)
    {
        UTEST_PRINTF("%d", static_cast<int>(s));
    }
};
...
template <> struct utest_type_deducer<float, false> {
    ...
    {
        UTEST_PRINTF("%f", static_cast<double>(f));
    }
};
...
template <> struct utest_type_deducer<unsigned int, false> {
    static void _(const unsigned int i)
    {
        UTEST_PRINTF("%u", i);
    }
};
...
template <> struct utest_type_deducer<unsigned long, false> {
    static void _(const unsigned long i)
    {
        UTEST_PRINTF("%lu", i);
    }
};
...
template <> struct utest_type_deducer<long long, false> {
    static void _(const long long i)
    {
        UTEST_PRINTF("%lld", i);
    }
};
...
template <> struct utest_type_deducer<unsigned long long, false> {
    static void _(const unsigned long long i)
    {
        UTEST_PRINTF("%llu", i);
    }
};
...
#define UTEST_I(FIXTURE, NAME, INDEX)                                          \
    UTEST_EXTERN struct utest_state_s utest_state;                             \
    static void utest_run_##FIXTURE##_##NAME##_##INDEX(int *,                  \
                                                       struct FIXTURE *);      \
        ...
        for (i = 0; i < (INDEX); i++) {                                        \
            ...
            if (utest_state.tests) {                                           \
                utest_state.tests[index].func =                                \
                    &utest_i_##FIXTURE##_##NAME##_##INDEX;                     \
                utest_state.tests[index].index = i;                            \
                utest_state.tests[index].name = name;                          \
                iUp = UTEST_CAST(utest_uint64_t, i);                           \
                UTEST_SNPRINTF(name, name_size, "%s/%" UTEST_PRIu64,           \
                               name_part, iUp);                                \
            } else if (name) {                                                 \
                free(name);                                                    \
            }                                                                  \
        ...
        while (('\0' != *filter_cur) && ('\0' != *testcase_cur)) {
            if ('*' == *filter_cur) {
                ...
                while (('\0' != *filter_cur) && ('\0' != *testcase_cur)) {
                    if ('*' == *filter_cur) {
                        /*
                           we found another wildcard (filter is something like
                           *foo*) so we exit the current loop, and return to the
                           parent loop to handle the wildcard case
                        */
                        break;
...
int utest_main(int argc, const char *const argv[])
{
    ...
    for (index = 1; index < UTEST_CAST(size_t, argc); index++) {
        ...
        if (0 == UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
            printf(
                ...
                "  --list-tests            List testnames, one per line. "
                "Output "
                "names can be passed to --filter.\n");
            printf("  --output=<output>       Output an xunit XML file to the "
                   "file "
                   "specified in <output>.\n"
                   "  --enable-mixed-units    Enable the per-test output to "
                   "contain "
                   "mixed units (s/ms/us/ns).\n"
                   "  --random-order[=<seed>] Randomize the order that the "
                   "tests are "
                   "ran in. If the optional <seed> argument is not provided, "
                   "then a "
            ...
            utest_state.output =
                utest_fopen(argv[index] + strlen(output_str), "w+");
        } else if (0 ==
                   UTEST_STRNCMP(argv[index], list_str, strlen(list_str))) {
            ...
            // because I really want to avoid using C's rand() because that'd
            // mean our random would be affected by any srand() usage by the
            // user (which I don't want).
    ...
    if (random_order) {
        // Use Fisher-Yates with the Durstenfield's version to randomly re-order
        // the tests.
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
...

        {
            ...
            if (enable_mixed_units) {
                for (unit_index = 0; UTEST_NULL != units[unit_index];
                     unit_index++) {
                    if (10000 > time)
                        break;
            ...
            if (UTEST_TEST_FAILURE == result) {
                printf("%s[  FAILED  ]%s %s (%" UTEST_PRId64 "%s)\n",
                       ...
                       utest_state.tests[index].name, time, units[unit_index]);
            } else if (UTEST_TEST_SKIPPED == result) {
